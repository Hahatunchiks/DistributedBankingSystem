...
int send_balance_history(const struct child_proc *child) {
    ...
    memcpy(history_message.s_payload, &child->history, sizeof(BalanceState) * child->history.s_history_len + sizeof(uint8_t) * 2);
...
int child_proc_work(struct child_proc *child) {
    ...
    if (send_balance_history(child) < 0) {
        printf("CANNOT SEND BALANCE TO PARENT %d -> parent\n", child->id);
        return -1;
    }
    return 0;
}
...
void k_proc_work(struct parent_proc *parent) {
    for (int i = 1; i <= parent->child_proc_count; i++) {
        ...
        while (1) {
            int result = receive_full(parent->parent_pipes_in[i].fd[0], &start_message);
            if (result < 0) {
                if(errno == EAGAIN || errno == EWOULDBLOCK) continue;
            }
            break;
    ...
    for (int i = 1; i <= parent->child_proc_count; i++) {
        Message done_message;
        while (1) {
            int result = receive_full(parent->parent_pipes_in[i].fd[0], &done_message);
            if (result < 0) {
                if(errno == EAGAIN || errno == EWOULDBLOCK) continue;
            }
            break;
    ...
    for (int i = 1; i <= parent->child_proc_count; i++) {
        Message history_message;
        while (1) {
            int result = receive_full(parent->parent_pipes_in[i].fd[0], &history_message);
            if (result < 0) {
                if(errno == EAGAIN || errno == EWOULDBLOCK) continue;
            }
            break;
