    ...
    while (1) {
        ...
        if(new_message.s_header.s_type == STOP) {
            ...
            if(received_transfer.s_src == child->id) {
                ...
                sprintf(log_transfer, log_transfer_out_fmt, child->balance.s_time, received_transfer.s_src, received_transfer.s_amount, received_transfer.s_dst);
                log_event(events_log, log_transfer);
            } else if(received_transfer.s_dst == child->id) {
                sprintf(log_transfer, log_transfer_in_fmt, child->balance.s_time, received_transfer.s_dst, received_transfer.s_amount, received_transfer.s_src);
                log_event(events_log, log_transfer);
...
void k_proc_work(struct parent_proc *parent) {
    for (int i = 0; i < parent->child_proc_count; i++) {
    ...
    bank_robbery(parent, (local_id) (parent->child_proc_count - 1));
...
int main(int argc, char **argv) {
    ...
    if (proc_id != 0) {
        ...
        parent.id = (local_id) (proc_count + 1);
